...
def train_discrim(discrim, memory, discrim_optim, demonstrations, args):
	...
	states = torch.Tensor(states).view(-1,10,7)
	actions = torch.Tensor(actions)
		
	criterion = torch.nn.BCELoss()

	demo_state = torch.Tensor(demonstrations[:,:70]).view(-1,10,7)
	demo_action = torch.Tensor(demonstrations[:,70:])

	for _ in range(args.discrim_update_num): 
		learner = discrim(states,actions) # state랑 action을 붙여서 사용
		
		# demonstrations = torch.Tensor(demonstrations)
		expert = discrim(demo_state, demo_action)
	...
	expert_acc = ((discrim(demo_state, demo_action) < 0.5).float()).mean()
